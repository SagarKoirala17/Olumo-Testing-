...
Cypress.Commands.add('EditQuestion', (index) => {
  ...
  cy.get(selector).clear().type(questions[index])
  ...
  cy.get('.visible > .item').should('be.visible').then(($items) => {
    ...
    if (questionType === 'Multiple Choice') {
      for (let j = 0; j < multiple.length; j++) {
        cy.get(`:nth-child(${index + 3}) > .olumo-form-element-section > .olumo-answer-list > :nth-child(${j + 2}) > .olumo-answer-input-field > .olumo-input-field-section > .olumo-default-input-field`).clear().type(multiple[j])
      }
    }
  })
})
